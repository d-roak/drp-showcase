...
export default function GridMenu(props: { node: DRPNode; setGrid: (grid: GridDRP | null) => void; setRerender: (rerender: number) => void }) {
    ...
    async function connectGrid() {
        ...
        if (!drpId) return;
        try {
            const drpObject = await props.node.createObject(new GridDRP(), drpId, undefined, true);
            const drp = drpObject.drp as GridDRP;
            createConnectHandlers();
            drp.addUser(props.node.networkNode.peerId);
            setGridId(drpId);
