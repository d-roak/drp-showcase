...
export default function Header(props: { node: DRPNode }) {
    ...
    const [bootstraps, setBootstraps] = useState<string[]>(props.node.networkNode.getBootstraps());
    useEffect(() => {
    ...
    return (
        <header className="mx-auto w-fit mt-10">
            ...
            <div className="mb-10">
                ...
                <div className="flex gap-2 items-center">
                    <p>Your Multiaddr: {props.node.networkNode.getMultiaddrs() ? props.node.networkNode.getMultiaddrs()[0] : ""}</p>
                    <button
                        className="py-1"
                        type="button"
                        onClick={() => {
                ...
                <p>
                    ...
                    {bootstraps.map((node, index) => (
                        <span key={node}>
                            {formatPeerId(node.split("/").pop() ? node.split("/").pop()! : "")}
                            <button
                                className="px-2 py-1 bg-red-700 ml-2"
                                onClick={() => {
